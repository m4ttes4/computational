...
!=============== INITIAL CONDITIONS OF THE GRID ==================
    ...
    22 continue
    !______________________ RESULTS OF STELLAR WINDS _____________________________
...
!============= TIME EVOLUTION ================
 ...
 do j=1,5
       ...
       do while (time<tmax(j))      !!!! HERE STARTS THE TIME INTEGRATION !!!!!
        ...
        !_______________ WIND INJECTION __________________________
        if(winds .eqv. .true.) then
            do i=1,3
                v(i) = 1.d8
                t(i) = 1.d4
                !d(i) = m_lost/(4*pi*xa(i)**2*v(i))
                d(i) = d(i) + (m_lost*dtmin/vol) !dens = dens precedente + materia aggiunta
                e(i) = cv*d(i)*t(i)
                !e(i) = 0.5*m_lost*dtmin*v(i)**2                
            enddo           
        end if
    ...
    enddo !end time cycle
        ...
        print*, 'TIME PASSED == ', int(tmax(j)/yr), 'YEARS' , ' time-step',dtmin/yr
